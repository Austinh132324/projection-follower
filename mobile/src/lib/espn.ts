...
//
// Ported from the web app (web/src/espn.ts). Uses ESPN's public (unofficial,
// undocumented) JSON endpoints. On a native device these fetch directly with no
// CORS restriction, so — unlike the browser build — there's no backend proxy
// fallback. Shapes are stable but not contractual, so every access is defensive.
//
